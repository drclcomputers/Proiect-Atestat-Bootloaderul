	...
	articles := []struct{ slug, title, summary, content string }{
{
			...
			`Dezvoltarea unui sistem de operare (sau măcar a primului său program, bootloaderul) începe rar cu o instrucțiune <code>jmp</code>. Începe cu o dorință puternică de învățare, FOOOARTE multă răbdare și zile sau săptămâni întregi de citit cărți, articole și urmărit tutoriale indiene la 11 noaptea. După, urmează configurarea unui mediu de lucru predictibil: un assembler, un emulator și un folder în care poți recompila în câteva secunde. Fără astea, fiecare încercare înseamnă să copiezi un fișier binar pe un stick și să repornești un calculator real (indiferent că e un 386 sau Ryzen 9 9950X) — lent spre foarte lent și, dacă greșești semnătura de boot, frustrant căci nu știi ce nu merge și de ce nu afișează nimic.
